...
class Rollbar {
    ...
    public static $ignore_php_error = false;
    ...
    public static function report_message($message, $level = 'error', $extra_data = null, $uuid = null) {
        if (self::$instance == null) {
            return;
        }
        self::$instance->report_message($message, $level, $extra_data, $uuid);
    }
    ...
    public static function start_ignore() {
        Rollbar::$ignore_php_error = true;
    }

    public static function stop_ignore() {
        Rollbar::$ignore_php_error = false;
    }
}
...
class RollbarNotifier {
    ...
    public function report_message($message, $level = 'error', $extra_data = null, $uuid=null) {
        try {
            $this->_report_message($message, $level, $extra_data, $uuid);
        } catch (Exception $e) {
            try {
    ...
    private function _report_message($message, $level, $extra_data, $uuid=false) {
        if (!$this->check_config()) {
            return;
        }
        ...
        $trace = $this->build_php_backtrace();
        $extra_data['trace'] = $trace;
    ...
    private function build_php_backtrace() {
        $arg = null;
        if (defined('DEBUG_BACKTRACE_IGNORE_ARGS')) $arg = DEBUG_BACKTRACE_IGNORE_ARGS;
