...
function support_attachment_create(obj) {
    ...
    var html = '<div class="row"><div class="col-sm-4"><p><a href="' + url + '">' + filename + '</a> (' + obj.mimetype + ', ' + size + ')</p></div><div class="col-sm-4"><p style="color: #E08B5B;" id="'+atid+'">File attached.</p></div></div>';
...
function support_attachment(obj) {
   ...
   atid = "atrfile"+atid.toString()
   var size = bytesToSize(obj.size);
   var html = '<div class="row"><div class="col-sm-4"><p><a href="' + url + '">' + filename + '</a> (' + obj.mimetype + ', ' + size + ')</p></div><div class="col-sm-4"><p style="color: #E08B5B;" id="'+atid+'">Please wait..</p></div></div>';
   var blob = JSON.stringify({inkurl: obj.url, filename: filename, url: url, size: obj.size, mime: obj.mimetype});
   var input = '<input type="hidden" name="attachment[]" value="' + htmlentities(blob) + '" />';
